...
class GCNSingleHead(nn.Module):
    ...
    def message_func(self, edges):
        return {'m': edges.src['Wh']}

    def reduce_func(self, nodes):
        return {'Wh': torch.sum(nodes.mailbox['m'], dim=1)}

    def forward(self, g, feature):
        g = g.local_var()
        Wh = self.dropout(feature)
        Wh = self.linear(Wh)
        g.ndata['Wh'] = Wh
        g.update_all(self.message_func, self.reduce_func)
        Wh = g.ndata['Wh']
        return self.activation(Wh)
...
class HeteroGATSingleHead(nn.Module):
    def __init__(self, g, in_feats, out_feats, type2mask, activation, residual, num_heads, dropout_prob, merge):
        ...
        self.Q_aggr = nn.Linear(aggr_out_feats, 2)
        nn.init.xavier_uniform_(self.W_aggr.weight)
        nn.init.xavier_uniform_(self.D_aggr.weight)
        nn.init.xavier_uniform_(self.Q_aggr.weight)
    ...
    def forward(self, sub_graphs, feature):
        with self.g.local_scope():
            ...
            DT_Hetero_Wh = self.g.ndata[f'hetero_Wh_1']
            TD_Hetero_Wh = self.g.ndata[f'hetero_Wh_2']
            TW_Hetero_Wh = self.g.ndata[f'hetero_Wh_3']
            WT_Hetero_Wh = self.g.ndata[f'hetero_Wh_4']
            WW_Hetero_Wh = self.g.ndata[f'hetero_Wh_5']
            D_alpha = F.softmax(self.D_aggr(torch.cat((DD_Hetero_Wh, DT_Hetero_Wh), dim=1)), dim=1)
            Q_alpha = F.softmax(self.Q_aggr(torch.cat((TD_Hetero_Wh, TW_Hetero_Wh), dim=1)))
            W_alpha = F.softmax(self.W_aggr(torch.cat((WT_Hetero_Wh, WW_Hetero_Wh), dim=1)))
            D_Hetero_Wh = self.aggr(DD_Hetero_Wh, DT_Hetero_Wh, D_alpha)
            Q_Hetero_Wh = self.aggr(TD_Hetero_Wh, TW_Hetero_Wh, Q_alpha)
            W_Hetero_Wh = self.aggr(WT_Hetero_Wh, WW_Hetero_Wh, W_alpha)
            Hetero_Wh = D_Hetero_Wh
            # Q_Hetero_Wh[self.type2mask["feature"]] = W_Hetero_Wh[self.type2mask["feature"]]
            Hetero_Wh[self.type2mask["topic"]] = Q_Hetero_Wh[self.type2mask["topic"]]
